  ...
  module Visitors
    ...
    describe "the oracle visitor" do
      ...
      it "modifies order when there is distinct and first value" do
        # *sigh*
        _select = "DISTINCT foo.id, FIRST_VALUE(projects.name) OVER (foo) AS alias_0__"
        stmt = Nodes::SelectStatement.new
        stmt.cores.first.projections << Nodes::SqlLiteral.new(_select)
        stmt.orders << Nodes::SqlLiteral.new("foo")
        sql = compile(stmt)
        sql.must_be_like %{
          SELECT #{_select} ORDER BY alias_0__
        }
      end
      ...
      it "splits orders with commas" do
        ...
        sql = compile(stmt)
        sql.must_be_like %{
          SELECT #{_select} ORDER BY alias_0__, alias_1__
        }
      end
      ...
      it "splits orders with commas and function calls" do
        ...
        sql = compile(stmt)
        sql.must_be_like %{
          SELECT #{_select} ORDER BY alias_0__ DESC, alias_1__
        }
      end
