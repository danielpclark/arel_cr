...
module Arel
  ...
  describe "select manager" do
    describe "backwards compatibility" do
      ...
      describe "_as" do
        it "makes an AS node by grouping the AST" do
          manager = Arel::SelectManager.new
          _as = manager._as(Arel.sql("foo"))
          assert_kind_of Arel::Nodes::Grouping, _as.left
          assert_equal manager.ast, _as.left.expr
    ...
    it "should hand back froms" do
      ...
      assert_equal [] of U, relation.froms
    end
    ...
    describe "joins" do
      ...
      it "can have a non-table alias as relation name" do
        ...
        counts = comments.from.
          ...
          project(
            comments[:user_id]._as("user_id"),
            comments[:user_id].count._as("count")
          )._as("counts")
